...
export function buildPdfSkippedNote(): string {
  ...
  if (process.env.VERCEL === '1') {
    return 'Vercel 서버리스에는 LibreOffice가 없어 기본적으로 PDF를 만들지 않습니다. PPTX(slides.pptx)를 내려받은 뒤 로컬에서 변환하거나, 이 프로젝트를 로컬(`npm run dev` 또는 `./start_server.sh`)로 띄우면 PDF까지 받을 수 있습니다. 실험적으로는 환경 변수 ENABLE_PDF_ON_VERCEL=1을 설정할 수 있으나, 용량·실행 시간 제한으로 실패하기 쉽습니다.';
  }
  return '';
}
